...
│       ├── api/          # down, single (non-blue-green) API
│       ├── patches/
│       └── kustomization.yaml
...
behavioral differences (blue-green vs. single Deployment, resource sizing,
ConfigMap values) live in each overlay's `patches/`. Render either environment
locally to see exactly what gets applied:
...
There is no internal/self-signed TLS chain in this cluster anymore — MinIO
(previously the only consumer of it) now runs external to k3s with a
publicly-trusted certificate (see
...
> in the ConfigMap. The same applies to MinIO: `MINIO_ENDPOINT`/
> `MINIO_EXTERNAL_ENDPOINT` must point at the external MinIO host — see
> [documentation/storage/infrastructure.md](../../documentation/storage/infrastructure.md).
> Only `MINIO_BUCKET_NAME`/`MINIO_USE_SSL` stay in the ConfigMap.
...
| `STAGING_MINIO_ENDPOINT` | MinIO host — self-managed, external to k3s (see [documentation/storage/infrastructure.md](../../documentation/storage/infrastructure.md)) |
| `STAGING_MINIO_EXTERNAL_ENDPOINT` | Public-facing MinIO hostname used to sign presigned URLs for the browser |
...
MinIO runs external to this cluster, self-managed, with a publicly-trusted
TLS certificate — there is no in-cluster Deployment, Service, PVC, or
cert-manager Certificate for it anymore. See
[documentation/storage/infrastructure.md](../../documentation/storage/infrastructure.md)
for the full topology, setup runbook, and backup/restore procedure.
...
#### 2. `ghcr-pull-secret` not found or expired
