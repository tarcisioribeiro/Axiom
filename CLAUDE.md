...
**Large flat apps**: despite the domain breadth below, `library`, `security` and `personal_planning` are each a single flat app — one `models.py`/`views.py` per app, **not** split into sub-package directories; the groupings named below are logical sections within that one file, not importable sub-modules. `library` covers the core catalog (`books`/`authors`/`publishers`/`readings`/`summaries`, plus `ReadingGoal`/`LiteraryTypeGoal`/`BookHighlight`) and the "Intellect" extension for structured learning (`Course`/`CourseModule`/`CourseLesson`/`CourseSession`, `Skill`/`SkillHistory`, `FlashCard` spaced repetition via the SM-2 algorithm generated from highlights, `KnowledgeLink` connecting books/authors/courses/skills into a knowledge graph, `IntellectBadge`). `security` covers the password vault (`passwords`, `stored_cards`, `stored_accounts`, `archives`, `activity_logs`) plus vault config/session keys, health snapshots, alert configuration, and credential-sharing tokens. `personal_planning` covers habits/goals (`services/instance_generator.py` lazily generates `TaskInstance`s from `RoutineTask` templates — it does not modify already-generated instances), a gamification bounded context (`GamificationProfile`, XP transactions, streaks, `Badge`/`UserBadge` — unrelated to `library`'s `IntellectBadge` despite similar vocabulary), an emotional-wellness "Wellness Center" (`EmotionalCheckin`, crisis-mode `CrisisImpulseLog`, a `WellnessIntervention` catalog, AI-generated `WellnessWeeklyReport`), workout tracking (exercises, workout plans, days, sessions, sets), and nutrition tracking (foods, meal types, menu options, meal logs) — all under the same app. `ExerciseDatasetEntry` vendors the public `hasaneyldrm/exercises-dataset` catalog (GitHub) — metadata + GIF/thumbnail media hosted on MinIO, populated once per environment via `manage.py import_exercise_dataset`. `Exercise.dataset_entry` (nullable FK) is the only source of exercise cover images — there is no free-form image upload; users pick one via a search picker in the frontend. The GIF propagates read-only through `WorkoutExercise` and `WorkoutSessionExercise` via `gif_url`/`thumbnail_url` `SerializerMethodField`s that resolve the FK chain (`WorkoutExercise.exercise` → `Exercise.dataset_entry`; `WorkoutSessionExercise.exercise` → `WorkoutExercise` → ...), proxied through `exercises/<pk>/gif|thumbnail/` (mirrors the `library` book-cover MinIO redirect pattern).
...
**Auth & session**: `lib/main.dart` (`AxiomMobileApp`) wires up `ThemeController`, `ApiEnvironmentController`, `ApiClient` and `SessionController`, boots via `GET /me/`, then hands off to a `go_router` shell. Login: `services/auth_service.dart` calls `authentication/token/` (with 2FA follow-up via `users/2fa/verify/`) through `services/api_client.dart`, a `dio` client with a `PersistCookieJar` — same httpOnly-cookie session model as the web app, since the backend never returns JWTs in the response body. A refresh interceptor mirrors the web `api-client.ts`. `config/api_environment.dart` switches between the local Docker API and the production VPS (`https://axiom.tjtux.duckdns.org`) at runtime. `theme/` implements all 16 web theme variants (`app_themes.dart`, transcribed from `apps/frontend/src/index.css`).
