...
export function LoanForm({ loan, accounts, members, onSubmit, onCancel, isLoading }: LoanFormProps) {
  ...
  const set = (patch: Partial<LoanFormData>) => setFormData((prev) => ({ ...prev, ...patch }));
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-2">
          ...
          <Input id="description" value={formData.description} onChange={(e) => set({ description: e.target.value })} required />
        </div>
        ...
        <div>
          ...
          />
          {loan && <p className="mt-1 text-xs text-muted-foreground">Calculado automaticamente a partir das receitas/despesas vinculadas</p>}
        </div>
        ...
        <div>
          ...
          <Input id="horary" type="time" value={formData.horary} onChange={(e) => set({ horary: e.target.value })} required />
        </div>
        ...
        <div>
          ...
          <Select value={formData.account.toString()} onValueChange={(value) => set({ account: parseInt(value) })}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
              {accounts.map((acc) => (
        ...
        <div>
          ...
          <Select value={formData.benefited.toString()} onValueChange={(value) => set({ benefited: parseInt(value) })}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
        ...
        <div>
          ...
          <Select value={formData.creditor.toString()} onValueChange={(value) => set({ creditor: parseInt(value) })}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
              {members.map((m) => <SelectItem key={m.id} value={m.id.toString()}>{m.name}</SelectItem>)}
            </SelectContent>
        ...
        <div>
          ...
          <Input id="installments" type="number" value={formData.installments} onChange={(e) => set({ installments: parseInt(e.target.value) })} />
        </div>

        <div>
          <Label htmlFor="interest_rate">Taxa de Juros (%)</Label>
          <Input id="interest_rate" type="number" step="0.01" value={formData.interest_rate ?? ''} onChange={(e) => set({ interest_rate: parseFloat(e.target.value) })} />
        </div>

        <div>
          <Label htmlFor="due_date">Data de Vencimento</Label>
          <DatePicker value={formData.due_date ?? undefined} onChange={(date) => set({ due_date: date ? formatLocalDate(date) : '' })} placeholder="Selecione a data de vencimento" />
        </div>

        <div>
          <Label htmlFor="payment_frequency">Frequência de Pagamento</Label>
          <Select value={formData.payment_frequency} onValueChange={(value) => set({ payment_frequency: value })}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
              {PAYMENT_FREQUENCIES.map((freq) => (
                <SelectItem key={freq} value={freq}>{translate('paymentFrequency', freq)}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
        ...
        <div>
          ...
          <Input id="late_fee" type="number" step="0.01" value={formData.late_fee} onChange={(e) => set({ late_fee: parseFloat(e.target.value) })} />
        </div>
          ...
          <Select value={formData.status} onValueChange={(value) => set({ status: value })}>
            ...
            <SelectContent>
              {LOAN_STATUSES.map((status) => (
                <SelectItem key={status} value={status}>{translate('loanStatus', status)}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
        ...
        <div>
          ...
          <Select
            ...
            onValueChange={(value) => set({ guarantor: value === 'none' ? null : parseInt(value) })}
          >
        ...
        <div className="col-span-2">
          ...
          <Textarea id="notes" value={formData.notes ?? ''} onChange={(e) => set({ notes: e.target.value })} rows={3} />
        </div>

        <div className="col-span-2">
          <Label htmlFor="contract_document">Documento do Contrato</Label>
          <Input id="contract_document" type="file" onChange={(e) => set({ contract_document: e.target.files?.[0] ?? null })} />
        </div>

        <div className="col-span-2 flex items-center gap-2">
          <input type="checkbox" id="payed" checked={formData.payed} onChange={(e) => set({ payed: e.target.checked })} className="rounded" />
          <Label htmlFor="payed" className="cursor-pointer">Empréstimo Pago</Label>
        </div>
      </div>
