...
from security.vault_crypto import VaultEncryptedField, clear_vault_key, set_vault_key
...
class VaultEncryptedFieldFallbackWarningTest(TestCase):
    ...
    def test_warning_emitted_on_vault_key_fallback(self):
        """
        Warning must be logged when vault-key decryption fails and app-key is used.
        """
        # Encrypt a value with the app key (simulates legacy data or data encrypted
        ...
        # decrypt_with_key raises DecryptionError, triggering the fallback path.
        unrelated_vault_key = Fernet.generate_key()
        set_vault_key(unrelated_vault_key)

        instance = _FakeModel(app_key_ciphertext)

        with self.assertLogs("security.vault_crypto", level=logging.WARNING) as cm:
            result = instance.secret

        self.assertEqual(result, plaintext)
        self.assertTrue(
            any("_FakeModel" in line and "secret" in line for line in cm.output),
